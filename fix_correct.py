...
for line in lines:
    if "set_title" in line or "plt.title" in line or "suptitle" in line:
        if "Source: WSN_Dataset" not in line:
            ...
            parts = line.split("'")
            if len(parts) >= 3:
                # The actual string is in parts[1] if simple quote, or parts[-2]
                # It's better to just do a string replace on the ending quote and parenthesis combination
                # e.g. "')" -> ". Source: WSN_Dataset')"
                pass
