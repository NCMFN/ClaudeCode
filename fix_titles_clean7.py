...
for line in lines:
    if "set_title" in line or "plt.title" in line or "suptitle" in line:
        if "Source: WSN_Dataset" not in line:
            # We want to replace the single quote that comes just before the closing parenthesis.
            line = re.sub(r"'(?=\s*\))", r". Source: WSN_Dataset'", line)
            # Or if it's a suptitle with a comma
            line = re.sub(r"'(?=\s*,)", r". Source: WSN_Dataset'", line)
