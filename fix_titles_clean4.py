...
apc_code = """
    ...
    try:
        ...
        print("Failed to save APC simulation df", e)
"""
...
for line in lines:
    if "set_title" in line or "plt.title" in line or "suptitle" in line:
        if "Source: WSN_Dataset" not in line:
            # We want to replace the last single quote that comes before the closing parenthesis.
            # Easiest way: replace "')" with ". Source: WSN_Dataset')"
            if "')" in line:
