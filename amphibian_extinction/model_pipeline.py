...
from imblearn.over_sampling import SMOTE # oops imblearn is imblearn not imbalanced_learn, let's fix
import os
...
print(f"Total species matched with all datasets: {len(df)}")

# Define target variable (classification into Threatened vs Non-Threatened)
# Threatened: CRITICALLY_ENDANGERED, ENDANGERED, VULNERABLE
# Non-Threatened: LEAST_CONCERN, NEAR_THREATENED
# Remove Extinct, Data Deficient, Not Evaluated to simplify the binary classification
...
sns.countplot(data=df, x='IUCN_Status', order=['LEAST_CONCERN', 'NEAR_THREATENED', 'VULNERABLE', 'ENDANGERED', 'CRITICALLY_ENDANGERED'])
plt.xticks(rotation=45)
plt.title('Distribution of Extinction Risk Categories')
plt.tight_layout()
...
# Convert everything to numeric and handle NaNs
for col in features_to_use:
    df[col] = pd.to_numeric(df[col], errors='coerce')

# We can impute missing values with the median
