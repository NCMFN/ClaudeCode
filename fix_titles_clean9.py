...
for line in lines:
    if "set_title" in line or "plt.title" in line or "suptitle" in line:
        if "Source: WSN_Dataset" not in line:
            # Let's use simple replace, but replace only the specific pattern
            # The pattern is usually closing a string, e.g. "')" or "',"
            if "')" in line:
                line = line.replace("')", ". Source: WSN_Dataset')")
            elif "'," in line:
                line = line.replace("',", ". Source: WSN_Dataset',")
            elif "')" not in line and "'," not in line:
                # for case: plt.title('Figure 11: 3D Scatter - Noise vs Energy vs Accuracy')
                # wait, that has "')"
                pass
