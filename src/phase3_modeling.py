...
def split_data(df):
    ...
    gss_val = GroupShuffleSplit(n_splits=1, train_size=0.75, random_state=RANDOM_SEED)
    train_df = df.iloc[train_idx].reset_index(drop=True)
    t_idx, v_idx = next(gss_val.split(train_df, groups=train_df['user_id']))

    splits['group'] = {
        'train': train_df.iloc[t_idx],
        'val': train_df.iloc[v_idx],
        'test': df.iloc[test_idx]
    }

    train_df, test_df = train_test_split(df, test_size=0.2, random_state=RANDOM_SEED, stratify=df['label'])
    train_df, val_df = train_test_split(train_df, test_size=0.25, random_state=RANDOM_SEED, stratify=train_df['label'])
    splits['random'] = {
        'train': train_df, 'val': val_df, 'test': test_df
    }

    splits['shift'] = {
        'train': df.iloc[:int(n*0.3)],
        'val': df.iloc[int(n*0.3):int(n*0.5)],
