...
import os
from config import CHRONO_TRAIN_FRAC, CHRONO_VAL_FRAC, STYLE
...
def generate_ablation_chart(ablation_df):
    for split in ablation_df['Split'].unique():
        ablation_df[ablation_df['Split']==split].to_csv(f"outputs/tables/ablation_study_{split}.csv", index=False)
