...
from config import RANDOM_SEED, EVASION_PEER_Z_MULTIPLIER, EVASION_GRAPH_DEGREE_MULTIPLIER, POISON_FRACTION
...
def test_adversarial(df, splits):
    ...
    X_evasion['peer_z_score'] = X_evasion['peer_z_score'] * EVASION_PEER_Z_MULTIPLIER
    X_evasion['graph_degree'] = X_evasion['graph_degree'] * EVASION_GRAPH_DEGREE_MULTIPLIER
    ...
    flip_idx = np.random.choice(len(y_poison), size=int(POISON_FRACTION * len(y_poison)), replace=False)
...
if __name__ == "__main__":
    ...
    from src.phase3_modeling import split_data
