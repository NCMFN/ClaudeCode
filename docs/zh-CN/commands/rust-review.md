---
description: 全面的Rust代码审查，涵盖所有权、生命周期、错误处理、不安全使用和惯用模式。调用rust-reviewer代理。
---
...
* 通过 `std::process::Command` 中未经验证的输入导致的命令注入
* 硬编码的凭据
* 通过原始指针导致的使用后释放
...
* 没有正当理由就抑制 clippy 警告
* 公共 API 缺少 `///` 文档
* 考虑对非 `must_use` 返回类型使用 `#[must_use]`，其中忽略返回值很可能是一个错误
