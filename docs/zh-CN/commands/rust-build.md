---
description: 逐步修复Rust构建错误、借用检查器问题和依赖问题。调用rust-build-resolver代理进行最小化、精准的修复。
---

# Rust 构建与修复

该命令调用 **rust-build-resolver** 代理，以最小的更改增量修复 Rust 构建错误。
...
| `cannot move out of` | 重构以获取所有权；仅在最后手段时克隆 |
