...
> 本文档在 [common/coding-style.md](../common/coding-style.md) 的基础上扩展了 C++ 特定内容。

## 现代 C++ (C++17/20/23)

* 优先使用**现代 C++ 特性**而非 C 风格结构
* 当类型从上下文显而易见时，使用 `auto`
* 使用 `constexpr` 表示编译时常量
...
* 使用 `std::make_unique` / `std::make_shared` 代替原始 `new`
...
查看技能：`cpp-coding-standards` 以获取全面的 C++ 编码标准和指南。
