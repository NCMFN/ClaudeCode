...
> 一个使用 PostgreSQL、Redis 和队列的 Laravel API 真实示例。
> 复制到你的项目根目录并根据你的服务进行自定义。

## 项目概述

**技术栈:** PHP 8.2+， Laravel 11.x， PostgreSQL， Redis， Horizon， PHPUnit/Pest， Docker Compose

**架构:** 模块化 Laravel 应用，采用 controllers -> services -> actions 结构， Eloquent ORM，队列处理异步任务，Form Requests 用于验证，API Resources 用于一致的 JSON 响应。
...
* 迁移文件提交到 git
* 使用 Eloquent 或查询构建器 (除非参数化，否则不使用原生 SQL)
* 为 `where` 或 `orderBy` 中使用的任何列创建索引
* 避免在服务中直接修改模型实例；优先通过仓库或查询构建器进行创建/更新
