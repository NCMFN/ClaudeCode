...
description: PyTorch深度学习模式和最佳实践，用于构建稳健、高效且可复现的训练管道、模型架构和数据加载。
origin: ECC
---

# PyTorch 开发模式

用于构建稳健、高效和可复现深度学习应用的 PyTorch 惯用模式和最佳实践。
...
始终编写可在 CPU 和 GPU 上工作的代码，避免硬编码设备。
...
| `torch.amp.autocast` | 混合精度实现 2 倍加速 |
...
**请记住**：PyTorch 代码应具备设备无关性、可复现性和内存意识。如有疑问，请使用 `torch.profiler` 进行分析，并使用 `torch.cuda.memory_summary()` 检查 GPU 内存。
