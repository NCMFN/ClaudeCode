...
description: Laravel安全最佳实践，涵盖身份验证与授权、验证、CSRF防护、批量赋值、文件上传、密钥管理、速率限制和安全部署。
origin: ECC
---
...
* 中间件提供基础防护（通过 `VerifyCsrfToken` 实现 CSRF 防护，通过 `SecurityHeaders` 设置安全标头）。
* 守卫和策略强制执行访问控制（`auth:sanctum`、`$this->authorize`、策略中间件）。
* 表单请求在数据到达服务层之前验证和整理输入（`UploadInvoiceRequest`）。
* 速率限制在认证控制之外增加了滥用防护（`RateLimiter::for('login')`）。
* 数据安全来自加密转换、批量赋值防护和签名路由（`URL::temporarySignedRoute` + `signed` 中间件）。
...
对于使用 Sanctum 的 SPA 认证，确保配置了有状态的请求：
