...
下方有 7 个可选的类别组。随后的详细确认列表涵盖了 8 个类别下的 45 项技能，外加 1 个独立的模板。使用 `AskUserQuestion` 配合 `multiSelect: true`：
