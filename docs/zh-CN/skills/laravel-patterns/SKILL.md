...
description: Laravel 架构模式、路由/控制器、Eloquent ORM、服务层、队列、事件、缓存以及生产应用中的 API 资源。
origin: ECC
---
...
* 使用显式绑定和范围绑定以保持路由可预测性；同时仍强制执行授权以进行访问控制。
* 倾向于使用类型化模型、强制转换和作用域，以保持领域逻辑的一致性。
* 将 I/O 密集型工作放在队列中，并缓存昂贵的读取操作。
...
使用具有清晰层次边界（HTTP、服务/操作、模型）的常规 Laravel 布局。
...
保持控制器精简。将编排逻辑放在服务中，将单一用途的逻辑放在操作中。
...
* 保持前缀和路径一致，以避免双重嵌套（例如，`conversation` 与 `conversations`）。
* 使用与绑定模型匹配的单个参数名（例如，`{conversation}` 对应 `Conversation`）。
...
如果你希望参数解析为不同的模型类，请定义显式绑定。对于自定义绑定逻辑，请使用 `Route::bind()` 或在模型上实现 `resolveRouteBinding()`。
...
对同一个筛选器，请使用全局作用域或命名作用域中的一种，除非你打算实现分层行为。
...
* 将密钥保留在 `.env` 中，将配置保留在 `config/*.php` 中
