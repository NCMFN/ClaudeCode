...
new_lines = []
for line in lines:
    if ("set_title" in line or "plt.title" in line or "suptitle" in line) and "Source:" not in line:
        # replace the last quote before a closing parenthesis or comma
        line = re.sub(r"'\)(?!\w)", r". Source: WSN_Dataset')", line)
        line = re.sub(r"',(?!\w)", r". Source: WSN_Dataset',", line)
        line = re.sub(r"'\)(?=\s)", r". Source: WSN_Dataset')", line) # catch cases with spaces
